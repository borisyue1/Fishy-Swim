...
class Gameplay: CCNode {
    ...
    func didLoadFromCCB(){
        ...
        timerSpawn = NSTimer.scheduledTimerWithTimeInterval(timeControl, target: self, selector: "spawnRandomStuff", userInfo: nil, repeats: true)
//        if firstJump == false {
//            tap1.visible = true
//            firstJump = true
//        }
...
//        }
    ...
    func spawnRandomStuff(){
        ...
        if rand < 0.25 {
            ...
            if fish.position.y > self.contentSizeInPoints.height {
                constant = -40
            }
            else { constant = 40 }
    ...
    func ccPhysicsCollisionBegin(pair: CCPhysicsCollisionPair!, fish: CCNode!, wildcard: CCNode!) -> ObjCBool {
        if gameOver {
            return false
        }
        triggerGameOver()
